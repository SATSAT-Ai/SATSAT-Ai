		...
		>
			<div className="bg-[#071f07] max-w-3xl w-full order-1 mx-auto rounded-[26px]">
				<div
					...
					tabIndex={0}
					className={cn(
						"flex w-full relative after:-left-px after:rounded-[26px] items-end h-full px-2 rounded-[26px]",
						{
							"border border-white/20": !glow || !showDefaultGlow,
						},
						{ glow: glow && showDefaultGlow }
					)}
				>
					<textarea
						disabled={loading}
						...
						className="w-full max-h-52 overflow-y-auto focus:ring-transparent border-none outline-non text-text-normal placeholder:text-white/70 pb-3 placeholder:text-text-14 rounded-lg bg-brand-green h-auto bg-transparent"
					...
					<div className="h-full grid place-content-center py-2">
						<button
							type="submit"
							disabled={loading || !watch("userMessage")}
							tabIndex={0}
							className="bg-brand-green-darker disabled:opacity-50 shadow-md enabled:hover:opacity-80 p-2 rounded-[26px]"
						>
			...
			<p className="text-center py-3 text-text-12 font-normal text-white/50">
